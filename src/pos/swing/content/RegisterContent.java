...
public class RegisterContent extends JPanel implements ActionListener, MouseListener, UpdateableContent{
	...
	public void actionPerformed(ActionEvent event) {
		if(event.getSource().equals(add)){
			...
			if(addDialog.getValidated()){
				String newElement = addDialog.getValidatedInput().trim();
				getAssociatedList().add(newElement);
				keys.write(identifier);
				parentWindow.writeToOutput(LogInfoGenerator.generateElementNewStatement(server.getUsername(), getType(), newElement));
				updateList();
		
				TreePath pathToNewElement = find((DefaultMutableTreeNode)tree.getModel().getRoot(), newElement);
		...
		if(event.getSource().equals(edit)){
			...
			if(addDialog.getValidated()){
				...
				parentWindow.writeToOutput(LogInfoGenerator.generateElementEditStatement(server.getUsername(), getType(), oldName, addDialog.getValidatedInput().trim()));
				updateList();
		...
		if(event.getSource().equals(delete)){
			...
			if(dialog.getValidated()){
				String newElement = dialog.getValidatedInput().trim();
				getAssociatedList().remove(toDelete);
				keys.write(identifier);
				server.updateInventoryElement(identifier, toDelete, newElement);
				parentWindow.writeToOutput(LogInfoGenerator.generateElementDeleteStatement(server.getUsername(), getType(), toDelete, newElement));
				updateList();
		...
		if(event.getSource().equals(shiftUp)){
			...
			UpdateableContentController.postUpdate(getUpdateIdentifierType(), "shifted");
		}
	...
	public void update(String updateIdentifier, String info) {
		if(updateIdentifier.equals(getUpdateIdentifierType())){
			updateList();
			updateButtons();
		}
	}
